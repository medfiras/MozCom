<?php
	...
	if ($_POST['submit'] == "step2") {
		...
		if (empty($email)){
			$error_mg[] = "E-mail can not be empty! Please re-enter.";	
		}
		
		if (empty($username)){
			$error_mg[] = "User Name can not be empty! Please re-enter.";	
		}
                ...
                <TR>
                    ...
                    <TD align=middle>
						...
						<?php } else {?>
							...
							<TR>
								<TD class=text align=left>
									<b>Step 3. Installation DataBaase </b>
								</td>
							</tr>
							<tr><td>&nbsp;</td></tr>	
							<tr>
							...
							<?php
							$adresse = $database_host;									
							$nom = $database_username;
							$motdepasse = $database_password;
							$database = $database_name;
							mysql_connect($adresse, $nom, $motdepasse);
							mysql_select_db($database);							
		
							$mdp = md5($userpassword);
							$cle = 4;
							mysql_query('INSERT INTO users (pseudo, mdp, email, dateinscri, dervisit,rang) VALUES ("'.$username.'" , "'.$mdp.'" , "'.$email.'" ,  "'.$temps.'" , "'.$temps.'",  "'.$cle.'" ) ') or die(mysql_error());
