...
$nombremail = mysql_result(mysql_query('SELECT COUNT(*) FROM users WHERE email = "'.$email.'" AND invit!=1'), 0);
...
mysql_query("UPDATE users SET  mdp ='".$mdp."' , pseudo='".$pseudo."',
			nom = '".$nom."' , prenom = '".$prenom."', avatar = '".$nomavatar."', 
			datenais = '".$annee."' , sexe = '".$sexe."', dateinscri = '".$temps."', dateinscri = '".$temps."', 			
			propos = '".$propos."', cle_activation = '".$cle."'
			WHERE email = '".$email."'") or die (mysql_error());
        	
		$password = $_POST['password'];
		$message = 'Bienvenue sur Mozilla Tunisia ,<br/>
		
		Vous êtes priés de conserver cet e-mail dans vos archives.<br/>
		
		Les informations concernant votre compte sur http://join.mozilla-tunisia.org sont les suivantes :<br/>
		...
		Pour valider votre compte http://join.mozilla-tunisia.org/valider.php?pseudo='.$pseudo.'&cle='.$cle.'<br/>
		
		Pour toute question veuillez nous contacter à contact@mozilla-tunisia.org<br/>
		
		Veuillez garder précieusement votre mot de passe car il est crypté chez nous.<br/>
		
		
	    Si par erreur vous ne vous êtes jamais inscrit sur Mozilla Tunisia , ou que vous avez reçu ce message par erreur ,envoyez un mail à contact@mozilla-tunisia.org en disant de supprimer le compte.<br/><br/><br/>
		Merci<br/>
		---------------------------------------------<br/>
		Mozilla Tunisia <br/>
		---------------------------------------------<br/>';
		$objet = "Inscription sur Mozilla Tunisia";
		$headers = "From: MozillaTunisia <hajjejfiras@gmail.com>\r\nReply-To: hajjejfiras@gmail.com";
